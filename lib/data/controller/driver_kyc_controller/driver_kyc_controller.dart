...
class DriverKycController extends GetxController {
  ...
  static bool _isRedirectingToAgreement = false;
  ...
  Future<void> beforeInitLoadKycData() async {
    ...
    try {
      ...
      if (model.remark?.toLowerCase() == 'agreement_required') {
        ...
        // Don't redirect - the login flow will handle agreement
        setStatusFalse();
        return;
      }
  ...
  Future<void> submitKycData() async {
    ...
    // Agreement is now submitted separately via /driver/driver-agreement
    ...
    if (response.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
      ...
      if (isAgreementError) {
        ...
        await Get.find<ApiClient>().sharedPreferences.setBool(SharedPreferenceHelper.volunteerAgreementAcceptedKey, false);
